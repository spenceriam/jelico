...
function normalizeGoogleModelVariantId(model: Partial<GoogleModelLike>): string {
  return getGoogleModelVariantId(model).toLowerCase()
}

function getGoogleModelVersion(model: Partial<GoogleModelLike>): number {
  const normalizedId = normalizeGoogleModelVariantId(model)
  const match = normalizedId.match(/gemini-[a-z-]*?(\d+(?:\.\d+)?)/)
  return match ? Number.parseFloat(match[1]) : 0
}

function isExperimentalGoogleModel(model: Partial<GoogleModelLike>): boolean {
  const normalizedId = normalizeGoogleModelVariantId(model)
  return normalizedId.includes('gemini-exp-') || normalizedId.includes('experimental')
}

export function isSpecializedGoogleModel(model: Partial<GoogleModelLike>): boolean {
  const normalizedId = normalizeGoogleModelVariantId(model)
  return GOOGLE_SPECIALIZED_MODEL_KEYWORDS.some((keyword) => normalizedId.includes(keyword))
}

function isPreviewGoogleModel(model: Partial<GoogleModelLike>): boolean {
  const normalizedId = normalizeGoogleModelVariantId(model)
  return normalizedId.includes('preview') || normalizedId.includes('experimental') || normalizedId.includes('-exp')
}

function getGoogleModelVariantWeight(model: Partial<GoogleModelLike>): number {
  const normalizedId = normalizeGoogleModelVariantId(model)
...
function getGooglePreviewRevisionParts(model: Partial<GoogleModelLike>): number[] {
  const normalizedId = normalizeGoogleModelVariantId(model)
...
export function supportsGoogleGenerateContent(model: Partial<GoogleModelLike>): boolean {
  ...
  return supportedGenerationMethods.length === 0 || supportedGenerationMethods.includes('generateContent')
}

export function compareGoogleModels(a: Partial<GoogleModelLike>, b: Partial<GoogleModelLike>): number {
...
export function sortGoogleModels<T extends Partial<GoogleModelLike>>(models: T[]): T[] {
  return [...models].sort(compareGoogleModels)
}

export function selectPreferredGoogleModels<T extends Partial<GoogleModelLike>>(models: T[]): T[] {
...
export function mergeDocumentedGeminiModels<T extends { id: string; name: string }>(models: T[]): T[] {
  const merged = [...models]
  const seenIds = new Set(merged.map((model) => model.id.trim().toLowerCase()).filter(Boolean))

  for (const fallback of DOCUMENTED_GEMINI_CHAT_MODELS) {
    if (seenIds.has(fallback.id)) continue
    merged.push(fallback as T)
    seenIds.add(fallback.id)
  }

  return sortGoogleModels(merged)
}
