export const DEFAULT_OPENAI_MODELS_ENDPOINT = 'https://api.openai.com/v1/models'
...
export function buildCompatibleModelsEndpointCandidates(
  ...
  if (normalizedBase.endsWith('/anthropic') || normalizedBase.endsWith('/anthropic/v1')) {
    try {
      ...
      // Keep the primary endpoint for malformed URLs; validation handles the user-facing error.
    }
  }
